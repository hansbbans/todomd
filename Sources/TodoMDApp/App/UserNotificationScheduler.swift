    ...
    final class UserNotificationScheduler {
        ...
        private func locationNotificationPlans(for record: TaskRecord) -> [PlannedLocationNotification] {
            ...
            guard status == .todo || status == .inProgress,
                  let locationReminder = record.document.frontmatter.locationReminder
            else {
                return []
            }

            let locationName = locationReminder.name?.trimmingCharacters(in: .whitespacesAndNewlines)
            let body: String = if let locationName, !locationName.isEmpty {
                locationReminder.trigger == .onArrival ? "Arriving at \(locationName)" : "Leaving \(locationName)"
            } else {
                locationReminder.trigger == .onArrival ? "Arrived at location" : "Left location"
            }
            ...
            return [
                PlannedLocationNotification(
                    ...
                    radiusMeters: min(1000, max(50, locationReminder.radiusMeters))
                ),
            ]
        }
        ...
        private nonisolated static func isManagedNotificationIdentifier(_ identifier: String) -> Bool {
